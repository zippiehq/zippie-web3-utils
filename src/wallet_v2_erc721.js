...
const { wallet_abi, redeemBlankCheck_abi, redeemBlankCheck_signature } = require('./contracts/zippieWalletErc721ContractAbi_v2.js')
...
function createBlankCheck(web3, account, ledger, tokenAddress, tokenId, message) {
...
function redeemBlankCheck(web3, blankCheck, recipientAddress) {
  ...
  const multisigContract = new web3.eth.Contract(wallet_abi, sender.contractAddress)
...
async function getTransactionData(web3, transactionHash) {
  ...
  const params = web3.eth.abi.decodeParameters(
    redeemBlankCheck_abi,
    txData.input.slice(redeemBlankCheck_signature.length)
  )
